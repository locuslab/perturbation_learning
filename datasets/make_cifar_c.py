...
for method_name in d.keys():
    ...
    for severity in range(1,6):
        ...
        for i,(img, label) in enumerate(zip(data.data, data.targets)):
            if i % 1000 == 0: 
                print(f"{i}/{len(data.data)}")
            labels.append(label)
            cifar_c.append(np.uint8(corruption(convert_img(img))))

        np.save(f'~/data/cifar-10-c-test/{d[method_name].__name__}_{severity}.npy',
                np.array(cifar_c).astype(np.uint8))

        np.save('~/data/cifar-10-c-test/labels.npy',
